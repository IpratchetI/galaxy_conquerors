...
- [Базовые классы](#базовые-классы)
  ...
  - [Расчет столкновений](#расчет-столкновлений)
  - [Sprite](#sprite)
  - [Vector](#vector)
...
Примечания
Класс Ship отвечает за управление кораблем игрока на игровом поле.
Методы moveLeft() и moveRight() изменяют координату X корабля для движения влево и вправо соответственно.
Метод stopMoving() останавливает движение корабля.
Методы startAnimation() и stopAnimation() запускают и останавливают анимацию движения корабля соответственно.
Метод update(canvasWidth: number) обновляет состояние корабля, учитывая ширину игрового поля.
Метод draw(ctx: CanvasRenderingContext2D, canvasHeight: number) отрисовывает корабль на игровом поле, используя контекст рисования и высоту игрового поля.
...
enemy.draw(ctx);
Примечания
Класс Enemy отвечает за управление врагом на игровом поле.
Враги могут двигаться вправо и влево по игровому полю.
При достижении края игрового поля враги смещаются вниз и меняют направление движения.
Метод draw(ctx: CanvasRenderingContext2D) отрисовывает врага на игровом поле с учетом его текущего положения и скорости.
...
Примечания
Класс Bullet отвечает за управление пулями на игровом поле.
Пули двигаются вверх с постоянной скоростью.
Метод update() обновляет координаты пули на основе ее скорости.
Метод draw(ctx: CanvasRenderingContext2D) отрисовывает пулю на игровом поле.
Метод isOutOfBounds() проверяет, находится ли пуля за границами игрового поля.
...
explosion.draw(ctx);
Примечания
Класс Explosion предоставляет возможность отображения анимации взрыва на игровом поле.
При создании экземпляра взрыва начинается проигрывание анимации автоматически.
Метод startAnimation() запускает проигрывание анимации взрыва.
Метод stopAnimation() останавливает проигрывание анимации взрыва.
Метод draw(ctx: CanvasRenderingContext2D) отрисовывает взрыв на игровом поле.
...
#### Страница игры
Компонент Game представляет собой страницу игры, где пользователь может играть в игровой процесс. Этот компонент содержит холст для отрисовки игры, кнопку для переключения в полноэкранный режим и всплывающее окно для паузы в игре.

Основные функциональности:
Отрисовка игрового процесса на холсте.
Обработка переключения в полноэкранный режим.
Всплывающее окно для паузы в игре.
Обновление максимального счета игрока и последнего счета после окончания игры.
Импорты:
React, useEffect, useRef, useState из react: для создания и управления компонентом.
Button из @components/Button: компонент кнопки.
useFullScreen из @hooks/useFullscreen: хук для управления полноэкранным режимом.
FullscreenIcon из @assets/icons/fullscreenButton.svg: иконка для кнопки переключения в полноэкранный режим.
useNavigate из react-router-dom: хук для навигации по страницам.
routerPaths из @/constants/routerPaths: пути страниц.
BreakGamePopup из ./components/BreakGamePopup: всплывающее окно для паузы в игре.
GameEngine из @/gameEngine/GameEngine: игровой движок.
useAppDispatch, useAppSelector, userState, updateScore из @/store: для обновления состояния пользовательского счета.
GameEngine.scss: стили для игрового движка.
